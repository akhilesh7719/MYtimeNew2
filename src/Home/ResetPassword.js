...
const ResetPassword = ({navigation, route}) => {
  ...
  useEffect(() => {
    console.log(
      '@@@@@@@@@@@@@verifyOtpScreenOtp222222=============',
      route.params.verifyOtpScreenOtp,
    );
    if (route.params && route.params.verifyOtpScreenOtp) {
      setVerifyToken(route.params.verifyOtpScreenOtp);
    }
  }, [route.params]);

  const updatePasswordRequest = async () => {
    const apiUrl = 'https://api.mytime.co.in/forget_passwords/forget_password';
    const data = {
      data: {
    ...
    try {
      ...
      if (response.ok) {
        ...
        alert('password forget successfully', responseData.message);
        ...
        alert(
          'password and confirm password does not match', response.errors,
        );
      }
    } catch (error) {
